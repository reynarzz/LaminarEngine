using Editor.Build;
using Engine;
using Engine.Utils;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace Editor.Views
{
    internal class AndroidBuildDrawer : PlatformBuildSettingsDrawer
    {
        private readonly MemberInfo[] _properties;
        public AndroidBuildDrawer()
        {
            var type = typeof(AndroidBuildSettings);
            _properties = ReflectionUtils.GetAllMembersWithAttribute<SerializedFieldAttribute>(type, true, true).ToArray();
        }
        public override void OnDraw(PlatformBuildSettings settings)
        {
            foreach (var member in _properties)
            {
                PropertyDrawer.DrawVars("AndroidBuildSettings", settings, member);
            }
        }
    }
}
