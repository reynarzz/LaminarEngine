    ...
    // Render
    {
        ...
        else
        {
            ...
            if (flags & ImGuiTreeNodeFlags_Bullet)
                ...
                // Reynarzz--
                RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), ColorConvertFloat4ToU32({ 0.5f, 0.5f , 0.5f , 1.0f }), is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0.70f);
                // ----
            }
            if (g.LogEnabled)
                LogSetNextTextDecoration(">", NULL);
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    if (g.Style.TabBarBorderSize > 0.0f)
    {
        ...
        // Reynarzz
        window->DrawList->AddRectFilled(ImVec2(tab_bar->SeparatorMinX, (y - g.Style.TabBarBorderSize) - 0.5f), ImVec2(tab_bar->SeparatorMaxX, y), GetColorU32(GetStyle().Colors[ImGuiCol_TabHorizontalLine]));
    }
    return true;
}
