...
namespace Engine.GUI
{
    ...
    [UniqueComponent, RequireComponent(typeof(RectTransform))]
    public class UICanvas : Component, IUpdatableComponent, IPreRenderable
    {
        ...
        {
            float x = (mousePosScreen.x / (float)Screen.Width) * (float)RectTransform.Rect.Width;
            float y = (mousePosScreen.y / (float)Screen.Height) * (float)RectTransform.Rect.Height;
            return new vec2(x, y);
        }
